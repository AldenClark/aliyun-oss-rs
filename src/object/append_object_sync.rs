...
impl AppendObjectSync {
    ...
    ///
    /// 若设置回调，将获得实时进度回调。
    pub fn send_file(mut self, file: impl Into<String>) -> Result<Option<String>, Error> {
        let file = file.into();
    ...
    ///
    /// 上传内存数据到 OSS。
    pub fn send_content(mut self, content: Vec<u8>) -> Result<Option<String>, Error> {
